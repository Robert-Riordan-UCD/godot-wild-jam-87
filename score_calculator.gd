...
func get_score() -> Dictionary[Hand, int]:
	...
	for edge in open_edges:
		_add_dot(Globals.tile_map.global_position + edge*Globals.TILE_SIZE*Globals.tile_scale)
		next_edges = next_edges + Globals.tile_map.get_next_edges(edge)
	
	var next_next_edges = []
	for next in next_edges:
		if next in open_edges: continue
		_add_dot(Globals.tile_map.global_position + next*Globals.TILE_SIZE*Globals.tile_scale, Color.RED)
		next_next_edges = next_next_edges + Globals.tile_map.get_next_edges(next)

	for next in next_next_edges:
		if next in open_edges: continue
		if next in next_edges: continue
		_add_dot(Globals.tile_map.global_position + next*Globals.TILE_SIZE*Globals.tile_scale, Color.GREEN)
...
func _add_dot(pos: Vector2, color: Color = Color.WHITE) -> void:
